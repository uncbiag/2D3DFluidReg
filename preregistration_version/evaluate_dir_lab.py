...
def eval_with_file(source_file, target_file, phi_file, dim, spacing, origin, plot_result=False):
    ...
    source_list = np.load("./data/points.npy")
...
def eval_with_data(source_list, target_list, phi, dim, spacing, origin, plot_result=False):
    ...
    # origin_list = np.repeat([[69,156,1],], target_list.shape[0], axis=0)-1.
    # dim = np.array([(440-69),(449-156),(112-1)])+1

    # target_list_t = torch.from_numpy((target_list-origin_list-1.)*spacing)
    target_list_t = torch.from_numpy((target_list)*spacing)
    ...
    phi_t = torch.from_numpy(utils.identity_map(np.array([202,143,167]).astype(np.int), 1./(np.array([202,143,167])-1.))).unsqueeze(0).double()

    warped_list_t = calc_warped_points(source_list_t, phi_t, dim, spacing)
    warped_list_t = warped_list_t + torch.from_numpy((origin_list+1.)*spacing)
    # warped_list_t = torch.from_numpy((source_list)*spacing)
...
if __name__ == "__main__":
    ...
    origin = np.flip(prop.item().get("crop")[0:3])-1
