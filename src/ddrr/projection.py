...
def generate_grids_multi(emi_pos, proj_size, resolution, sample_rate, near_plane, device, dtype):
        ...
        P0 = torch.mm(
            torch.linspace(0, near_plane, sr_w, device=device, dtype=dtype).unsqueeze(1), 
            torch.tensor([[0., 0., 1.]], device=device, dtype=dtype))
        I0 = torch.from_numpy(emi_pos).to(device).unsqueeze(1).unsqueeze(1).type(dtype)  # Nx1x1x3
        N = torch.tensor([0., 0., 1.], device=device, dtype=dtype)
        
        # Calculate direction vectors for each rays
        lin_x = torch.linspace(-d/2, d/2, steps=res_d)
        lin_y = torch.linspace(-h/2, h/2, steps=res_h)
        grid_x, grid_y = torch.meshgrid(lin_x, lin_y)
        I = torch.zeros((lin_x.shape[0], lin_y.shape[0], 3), device=device, dtype=dtype)
        I[:,:,0] = grid_x
        I[:,:,1] = grid_y
        I = torch.add(I,-I0) # N x res_0 x res_1 x 3
        dx = torch.norm(torch.mul(I, 1./I[:,:,:,2:3]), dim=3)
...
def project_grid_multi(emi_pos, resolution, sample_rate, obj_shape, spacing, device, dtype):
        ...
        # Calculate direction vectors for each rays
        lin_x = torch.linspace(-res_d/2, res_d/2-1, steps=res_d)
        lin_y = torch.linspace(-res_h/2, res_h/2-1, steps=res_h)
        grid_x, grid_y = torch.meshgrid(lin_x, lin_y)
        I = torch.zeros((lin_x.shape[0], lin_y.shape[0], 3), device=device, dtype=dtype)
        I[:,:,0] = grid_x
        I[:,:,2] = grid_y
        I = torch.add(I,-I0)
        dx = torch.norm(torch.mul(I, 1./I[:,:,:,1:2]), dim=3)
        I = I/torch.norm(I, dim=3, keepdim=True)
...
def calculate_projections(img, poses, resolution, sample_rate, spacing, device):
    ...
    grids, dx = generate_grids_multi(poses, resolution, sample_rate, I0.shape[2:], spacing, I0.device, I0.dtype)
    grids = to_image_coord(grids)
...
if __name__ == "__main__":
    grids, dx = generate_grids_multi(emi_pos=np.array([[0,0,915]]), proj_size=[155, 155], resolution=[512, 512], sample_rate=100, near_plane=700, device=torch.device('cuda'), dtype=torch.float32)