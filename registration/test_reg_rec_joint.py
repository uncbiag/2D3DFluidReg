...
from customMermaidOptimizer import CustomSimpleSingleScaleRegistration
...
def main(args):
    ...
    I0_proj = torch.from_numpy(np.load(preprocessed_folder+'/' + prefix + '_I0_proj.npy')).unsqueeze(0).to(device)
    
    # I0_rec = torch.rand_like(I0).to(device)
    # I0_rec = torch.ones(I0.shape).to(device)*0.1
    ...
    I1_rec = torch.ones(I1.shape).to(device)*I1.mean() #I1.clone()
    ...
    if lung_reg_params["affine"]["run"]:
        ...
        mermaid_params_affine['model']['registration_model']["similarity_measure"]["projection"]["sigma"] = 0.1

        affine_opt = CustomSimpleSingleScaleRegistration(I0_rec,
                                                    I1,
                                                    I0_proj,
                                                    mermaid_spacing,
                                                    sz,
        ...
        affine_opt.get_optimizer().set_save_fig(False)
    ...
    if lung_reg_params["deformable"]["run"]:
        ...
        mermaid_params_proj['optimizer']['single_scale']['nr_of_iterations'] = 50
        mermaid_params_proj['model']['registration_model']['type'] = "lddmm_shooting_map" # "svf_vector_momentum_map"#"lddmm_shooting_map"
        mermaid_params_proj['model']['registration_model']['similarity_measure']['sigma'] = 1
        ...
        mermaid_params_proj['model']['registration_model']["similarity_measure"]["projection"]["sigma"] = 1
        mermaid_params_proj['model']['registration_model']["similarity_measure"]["projection"]["lr"] = 0.5

        mermaid_params_proj['optimizer']['name'] = "sgd"
        mermaid_params_proj['optimizer']['sgd']['individual']['lr'] = 0.0000001
        opt = CustomSimpleSingleScaleRegistration(I0,
                                            I1_rec,
                                            I1_proj,
                                            mermaid_spacing,
                                            sz,
        ...
        opt.get_optimizer().set_visualize_step(1)
        opt.get_optimizer().set_save_fig(True)
        opt.get_optimizer().set_expr_name("reg_rec_joint")
        opt.get_optimizer().set_save_fig_path("./log/reg_rec_joint")
        ...
        # ###############################
        # # Save the results
        np.save(os.path.join(exp_path, prefix + "_lddmm_recon.npy"), opt.ISource.detach().cpu().numpy())
