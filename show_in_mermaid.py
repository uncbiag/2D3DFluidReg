...
class ImageViewer3D_Sliced_Grids(viewers.ImageViewer3D_Sliced):
    ...
    def show_grids(self):
        ...
        for d in range(0,self.sliceDim):
            plt.contour(phiSliced[d,:,:], np.linspace(0,self.data.shape[d],20),colors='lightgray',linestyles='solid', linewidths=0.7)
...
disp_file = "./data/disp_noMask_large.npy"
...
if SHOW_SDT:
    ...
    warped_ivx_grids = ImageViewer3D_Sliced_Grids( ax[2,0], warped, phi, 0, 'Z slice')
    warped_ivy_grids = ImageViewer3D_Sliced_Grids( ax[2,1], warped, phi, 1, 'X slice')
    warped_ivz_grids = ImageViewer3D_Sliced_Grids( ax[2,2], warped, phi, 2, 'Y slice')
...
plt.show()
# plt.savefig("./data/imageViewer.png")