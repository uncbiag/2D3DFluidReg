from medical_image_utils import load_IMG, win_scale
...
def plot_hist(args):
    ...
    img, mask, bbox = load_IMG(lung_reg_params["source_img"], shape, spacing, new_spacing)
    img = win_scale(img, 1000, 1000, np.int, [0, 255])
    plt.hist(img.flatten(), bins=100)
    plt.savefig("./log/dataset_analysis/hist.png")
